...
(function () {
    const doShim = function () {
        ...
        const handleSocketSendError = function (operationName = "send", error) {
            console.error(`Error during WebSocket ${operationName}:`, error || "Unknown error");
            if (solverSocket) {
        ...
        const exportPuzzleForSolving = function (includeCandidates) {
            ...
            if (getSolutionCap() > 1)
                puzzle.truecandidatesoptions.push(`truecandidatesnumsolutions=${getSolutionCap()}`);
            if (boolSettings["LogicalCandidates"]) puzzle.truecandidatesoptions.push("logical");
        ...
        const sendPuzzleDelayed = function (message) {
            if (nonce === message.nonce) {
                if (solverSocket) {
                    ...
                    console.log("Socket closed before delayed send for nonce:", message.nonce);
                    if (connectButton.title === "Calculating...") connectButton.title = "Connect";
                    if (cancelButton && (cancelButton.title === "Cancel" || cancelButton.title === "Cancelling...")) {
                        cancelButton.title = cancelButton.origTitle;
                    }
                }
            }
        ...
        const doCancelableCommand = function (force) {
            ...
            if (this.solverCommand !== "truecandidates") boolSettings["TrueCandidates"] = false;
            ...
            if (cancelButton === this) {
                ...
                if (
                    cancelButton &&
                    cancelButton !== this &&
                    (cancelButton.title === "Cancel" || cancelButton.title === "Cancelling...")
                ) {
                    cancelButton.title = cancelButton.origTitle;
                }
        ...
        let initCancelableButton = function (button, command) {
            ...
            } else if (button.title === "Cancel" || button.title === "Cancelling...") {
        ...
        let hookSolverButtons = function () {
            ...
            if (consoleSidebar && mainSidebar) {
                ...
                if (!solutionPathButton || !stepButton || !checkButton || !countButton || !solveButton) {
                    console.warn(
                ...
                trueCandidatesButton.x = estimateCountButton.x - buttonLH / 2 - buttonGap / 2;
        ...
        let showSolverButtons = function () {
            ...
            if (consoleSidebar) {
                if (estimateCountButton && !consoleSidebar.buttons.includes(estimateCountButton)) {
                    consoleSidebar.buttons.push(estimateCountButton);
                }
                if (estimateCountButton) numCustomButtons++;

                if (trueCandidatesButton && !consoleSidebar.buttons.includes(trueCandidatesButton)) {
                ...
                if (initialConsoleSection0Y === 0 && consoleSidebar.sections && consoleSidebar.sections.length > 0) {
                ...
                if (consoleSidebar.sections && consoleSidebar.sections.length > 0 && initialConsoleSection0Y > 0) {
        ...
        const lerpColor = function (a, b, amount) {
            ...
            const rr = ar + amount * (br - ar),
                rg = ag + amount * (bg - ag),
                rb = ab + amount * (bb - ab);
            const colStr = "000000" + ((rr << 16) + (rg << 8) + (rb | 0)).toString(16);
        ...
        const setCenterMarkColor = function (cell, numSols, candIdx, solutionCap) {
            ...
            if (numSols < 0) curCol = logicalSolutionColor;
            else if (realSolutionCap === 1) curCol = baseSolutionColor; // All black if cap is 1
                ...
                const normalizedAmount = Math.min(solutionCap - 2, clampedSols - 2) / Math.max(1, solutionCap - 2);
                curCol = lerpColor(twoSolutionColor, eightSolutionColor, normalizedAmount);
            }
        ...
        const handleInvalid = function (response) {
            if (response.type === "invalid") {
                if (response.message && response.message.length > 0) log(response.message);
        ...
        const handleEstimate = function (response) {
            ...
            log(`Solution Estimate (after ${response.iterations.toLocaleString()} iterations):`);
            log(`  ~ ${response.estimate} (Rel.Err: ${response.relErrPercent}%)`);
            log(`  Stderr: ${response.stderr}`);
            log(`  95% CI: [${response.ci95_lower}, ${response.ci95_upper}]`);
                ...
                try {
                    ...
                    socket.onmessage = function (msg) {
                        ...
                        if (!allowCommandWhenUndo[lastCommand] && changeIndex < changes.length - 1) {
                            console.log("Undo occurred, discarding response for command:", lastCommand);
                            if (cancelButton && cancelButton.solverCommand === lastCommand)
                                cancelButton.title = cancelButton.origTitle;
                            if (connectButton.title === "Calculating..." || connectButton.title === "Cancelling...")
                                connectButton.title = "Disconnect";
                            return;
                        }
                        try {
        ...
        const solutionCapButtonClick = function () {
            ...
            const newCap = prompt(`Enter solution cap (minimum 1):`, currentCap.toString());
            if (newCap !== null) {
        ...
        drawPopups = function (overlapSidebars) {
            ...
            if (overlapSidebars && popup === "solversettings") {
                ...
                const solutionCapButton = buttons.find(
                    (b) => b.modes && b.modes.includes("solversettings") && b.id === "SolutionCap"
                );
        ...
        for (let buttonData of settingsButtons) {
            if (!buttonData.heading) {
                ...
                if (
                    ...
                    !boolSettings.find((bs) => typeof bs === "string" && bs === buttonData.id)
                ) {
        ...
        createOtherButtons = function () {
            ...
            for (let name of extraSettingsNames) {
                ...
                if (idx > -1) {
                    ...
                    let dIdx = defaultSettings.findIndex((ds) => typeof ds === "object" && ds.id === name);
                    if (dIdx === -1) dIdx = defaultSettings.length - (boolSettings.length - idx);
                    if (dIdx > -1 && dIdx < defaultSettings.length) defaultSettings.splice(dIdx, 1);
                }
            }
            ...
            for (let name of extraSettingsNames) {
                if (!boolSettings.includes(name)) {
                    ...
                    boolSettings[name] = tempRem.hasOwnProperty(name) ? tempRem[name] : dVal;
                } else {
                    boolSettings[name] = tempRem.hasOwnProperty(name) ? tempRem[name] : boolSettings[name];
                }
            }
            buttons
                .filter(
        ...
        if (document.getElementById("previewTypeBox")) document.getElementById("previewTypeBox").style.top = "29%";
        ...
        cell = function (i, j, outside) {
                ...
                if (val <= size) {
                    if (!val) this.givenPencilMarks = [];
                        ...
                        if (this.givenPencilMarks.includes(val))
                            this.givenPencilMarks.splice(this.givenPencilMarks.indexOf(val), 1);
            ...
            c.showTop = function () {
                ...
                if ((currentTool === "Regions" && !previewMode) || this.value) this.origShowTopSS();
                else if (!previewMode) {
                    const TLInt =
                        constraints[cID("Killer Cage")].some((a) => a.value.length && a.cells[0] === this) ||
                        cosmetics[cID("Cage")].some((a) => a.value.length && a.cells[0] === this) ||
                        constraints[cID("Quadruple")].some((a) => a.cells[3] === this);
                    const TRInt = constraints[cID("Quadruple")].some((a) => a.cells[2] === this);
                    const BLInt = constraints[cID("Quadruple")].some((a) => a.cells[1] === this);
                    const BRInt = constraints[cID("Quadruple")].some((a) => a.cells[0] === this);
                    ctx.fillStyle = boolSettings["Dark Mode"] ? "#F0F0F0" : "#000000";
                    ...
                    if (!this.centerPencilMarkColors) {
                        ...
                        for (let a = 0; a < cPMA.length; a++)
                            ctx.fillText(
                                cPMA[a].join(""),
                                this.x + cellSL / 2,
                                this.y + cellSL * cMO - ((cPMA.length - 1) / 2 - a) * cellSL * 0.1666 * textScale
                            );
                    } else {
                        ...
                        for (let a = 0; a < cPMA.length; a++) {
                            ...
                            const y = this.y + cellSL * cMO - ((cPMA.length - 1) / 2 - a) * cellSL * 0.1666 * textScale;
                ...
                if (hGM) {
                    ...
                    for (let a = 0; a < gPMA.length; a++)
                        ctx.fillText(
                            gPMA[a].join(""),
                            this.x + cellSL / 2,
                            this.y + cellSL * 0.85 - ((gPMA.length - 1) / 2 - a) * cellSL * 0.1666 * textScale
                        );
                }
                if (
        ...
        importPuzzle = function (str, clearHist) {
            ...
            for (let i = 0; i < size; i++)
                for (let j = 0; j < size; j++) {
                    if (puz.grid[i][j].givenPencilMarks && puz.grid[i][j].givenPencilMarks.length > 0)
